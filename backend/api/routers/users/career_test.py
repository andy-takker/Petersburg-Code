...
career_test_router = APIRouter(prefix='/{user_id}/career-tests',
                               tags=['Профориентационные тесты пользователей'])
...
async def get_user_career_test(user_id: int, career_test_id: int,
                               user_repo: UserRepo = Depends()) -> Any:
    career_test = await user_repo.get_user_career_test(
        user_id=user_id,
...
async def post_user_career_test(user_id: int, skills: SkillList,
                                user_repo: UserRepo = Depends()) -> Any:
    ...
    career_test = await user_repo.save_career_test(user_id=user_id,
                                                   proektoria=p)
    return career_test
